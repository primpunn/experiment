...
Depth-assisted Kalman smoother (replaces B&L for Stages 1-2):
    Three measurement noise levels (R) per joint per frame:
      RGB valid (ArUco detected, not artifact) → R = R_RGB²   (low)
      RGB absent/artifact, depth available     → R = R_depth² (medium)
      Both unavailable                         → predict-only (no update)
    Equation: K_t = P̂_t Ĥ_t^T (Ĥ_t P̂_t Ĥ_t^T + R_t)^{-1}
              m_t = m̂_t + K_t(z_t^depth - Ĥ_t m̂_t)  [depth z when RGB absent]
...
R2 strategy (Gløersen & Federolf 2016, Eq. 5):
    Joints in R2_JOINTS (right_elbow) contribute to the SVD subspace only when
    valid.  SVD training rows with missing R2-joint data are discarded rather
    than imputed; Kalman estimates are NOT written back for absent R2 frames.
    Reference: Gløersen Ø. & Federolf P. (2016) PLoS ONE — "Predicting missing
    marker trajectories in human motion data using marker intercorrelations."

Output: pose_<joint>_processed.txt alongside originals.

Usage:
    conda activate massage
    python interpolate_depth_v2.py <session_dir>
    python interpolate_depth_v2.py <session_dir> --no-bl --no-ffnn --no-depth --dry_run
"""
...
# Use for high-occlusion markers that are not contact-point measurements.
    ...
    for t in range(n):
        ...
        for j in joints:
            if trans_all[j][t] is not None and not artifact_mask[j][t]:
                ...
                # R2: discard row rather than impute with mean
                row = None
                break
...
def run_pipeline(session_dir: str, use_bl: bool, use_ffnn: bool,
    ...
    poses, trans = apply_bl_or_fallback_depth(
        ...
        pointclouds=pointclouds, rot_method='slerp')
