#!/usr/bin/env python3
"""
Post-process a saved session: detect 4 artifact types then interpolate/repair.

Detection pipeline — Skurowski & Pawlyta (2022):
...
Interpolation (translation):
  Stages 1-2  Burke & Lasenby (2016) Low-Dimensional Kalman Smoothing
...
Interpolation (rotation — Haarbach et al. 2018 survey, Eqs. 14-18):
  Stage 1     SLERP  — C0, 2 anchors          (single peak, 1-5 frames)
  Stage 2     SQUAD  — C1, 4 keyframes         (heavy noise, 5-50 frames)
  Stages 3-4  CuBsp  — C2, cumulative B-spline (step/drift, >20 frames)
  Fallback chain: CuBsp -> SQUAD -> SLERP when context frames unavailable

Output: pose_<joint>_processed.txt alongside originals.

Usage:
    conda activate massage
    python interpolate_modify.py <session_dir>
    python interpolate_modify.py <session_dir> --no-bl --no-ffnn --dry_run
"""
...
import numpy as np
from scipy.ndimage import binary_closing, binary_opening, grey_closing, median_filter, uniform_filter1d
...
# Paper 1 §3.4.4 explicitly states: "Default parameters of Savitzky-Golay are
# the same as for heavy noise L=5, M=13" — so Stage 3 reuses NOISE_SG_* vars.
...
BL_MIN_JOINTS_TRAIN  = 2    # minimum valid joints per frame for SVD training
BL_ENERGY_THRESH     = 0.99 # gamma — SVD energy fraction to retain
BL_Q_SIGMA       = 0.01     # process noise sigma  (m)
BL_R_SIGMA       = 0.03     # measurement noise sigma  (m)

# ── R2 Strategy — Gløersen & Federolf (2016) ─────────────────────────────────
# Joints listed here contribute to the SVD subspace when valid,
# but are NOT filled/interpolated in frames where data is absent.
# Use for high-occlusion markers that are not contact points.
R2_JOINTS = ['right_elbow']
...
# N0 = 1 always (partition of unity); only N1, N2, N3 enter the curve formula.
# Verified: Nk = sum_{j>=k} b_j(u) where b_j are standard B-spline basis funcs.
...
def _detect_peaks_1d(x: np.ndarray) -> np.ndarray:
    ...
    win         = 2 * PEAK_MAX_SIZE + 3
    mov_sum     = uniform_filter1d(D_tilde,         size=win, mode='constant') * win
    mov_sum_abs = uniform_filter1d(np.abs(D_tilde), size=win, mode='constant') * win
...
def detect_heavy_noise(trans_joint: list) -> np.ndarray:
    n = len(trans_joint)
    mask = np.zeros(n, dtype=bool)
    for start, end in get_valid_runs(trans_joint):
        if end - start < 3:
...
def _detect_step_1d(x: np.ndarray) -> np.ndarray:
    ...
    # Same SG params as Stage 2: "Default params same as heavy noise L=5, M=13"
    D1 = np.append(np.diff(x), 0.0)
    D1_HP = _sg_highpass(D1, NOISE_SG_WIN, NOISE_SG_POLY)
...
def burke_lasenby_smooth(trans_all: dict, artifact_mask: dict):
    ...
    # Training set: frames where >= BL_MIN_JOINTS_TRAIN joints are valid.
    # Missing joints are imputed with their temporal mean so the subspace
    # can be estimated even when arms frequently occlude each other.
    train_rows = []
    for t in range(n):
        valid_count = sum(
            1 for j in joints
        ...
        for j in joints:
            if trans_all[j][t] is not None and not artifact_mask[j][t]:
                ...
                row = None   # R2: discard entire row rather than impute with mean
                break
            else:
    ...
    for t in range(n):
        ...
        for j_idx, j in enumerate(joints):
            if trans_all[j][t] is None or artifact_mask[j][t]:
                if j in R2_JOINTS:
                    pass   # R2: keep None — do not fill frames with no real data
                else:
...
# =============================================================================
# B&L or fallback dispatcher  (Stages 1 & 2)
# =============================================================================
...
def apply_bl_or_fallback(poses_all: dict, trans_all: dict,
    ...
    for j in ARM_JOINTS:
        ...
        if bl_result is not None:
            ...
            if j in R2_JOINTS:
                # R2: only fill confirmed artifact frames, not originally-None frames.
                # Rotation is still corrected for artifact frames that were detected.
                r2_fill_mask = mask.copy()
                if r2_fill_mask.any():
                    new_poses[j] = fill_rotations(
                        new_poses[j], r2_fill_mask, method=rot_method)
            else:
...
def run_pipeline(session_dir: str, use_bl: bool, use_ffnn: bool, dry_run: bool):
    frames = sorted(
        [d for d in os.listdir(session_dir) if d.startswith('frame_')],
        key=lambda x: int(x.split('_')[1])
    )
    ...
    counts = {j: dict(peak=0, noise=0, step=0, drift=0) for j in ARM_JOINTS}
    ...
    poses, trans = apply_bl_or_fallback(poses, trans, mask_peak, use_bl,
                                        rot_method='slerp')

    # ── Stage 2: Heavy Noise — rotation: SQUAD (C1, 5-50 frames) ─────────────
    print("Stage 2: Detecting heavy noise...")
    mask_noise = {j: detect_heavy_noise(trans[j]) for j in ARM_JOINTS}
    for j in ARM_JOINTS:
        counts[j]['noise'] = int(mask_noise[j].sum())
        cumulative[j]  |= mask_noise[j]
        interp_mask[j] |= mask_noise[j]
    poses, trans = apply_bl_or_fallback(poses, trans, mask_noise, use_bl,
                                        rot_method='squad')

    # ── Stage 3: Step Changes — rotation: CuBsp (C2, >20 frames) ────────────
    # exclude_mask prevents the detector from seeing B&L-filled or previously
    # artifact-corrected frames as genuine signal (fixes right_elbow over-detection)
    ...
    print(f"\n{'Joint':<{COL}} {'NaN bef':>10} {'NaN aft':>10} "
          f"{'Peak':>6} {'Noise':>6} {'Step':>6} {'Drift':>6} {'Recov':>6}")
    print('─' * 78)
    for j in ARM_JOINTS:
        ...
        print(f"{j:<{COL}} {nb:>4}({pct_b:4.1f}%) {na:>4}({pct_a:4.1f}%) "
              f"{ac['peak']:>6} {ac['noise']:>6} {ac['step']:>6} "
...
def main():
    parser = argparse.ArgumentParser(
        description='4-stage artifact detection + repair for arm pose files')
